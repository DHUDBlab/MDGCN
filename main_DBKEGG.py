...
def make_args():
    ...
    parser.add_argument('--dataset', type=str, default='DB-KEGG')
    parser.add_argument('--batch', type=int, default=20000, metavar='N', help='input batch size for training')
    ...
    parser.add_argument('--layers', type=int, default=14+3, help='the numbers of GCN layer')
    ...
    parser.add_argument('--metareg', type=float, default=0.13, help='weight of loss with reg')
...
class Hope:
    def __init__(self, args, _data, _distanceMat):
        ...
        self.train_loader = dataloader.DataLoader(MyTransData(self.trainMat), batch_size=args.batch, shuffle=True,
                                                  num_workers=0)
        self.train_loss = []
        self.test_auroc = []
        self.test_aupr = []
    ...
    def _test(self):
        with t.no_grad():
            ...
            # 计算 F1 分数
            epF1 = f1_score(labels, binary_preds)'''
            ...
            return epAuc, epAupr, labels, preds, epF1
    ...
    def run(self):
        ...
        for e in range(args.epochs + 1):
            self.cur_Epoch = e
            loss = self._train()
            AUROC, AUPR, y_true, y_score, F1 = self._test()
            epoch_time = self.train_times[-1]
            log("epoch %d/%d, loss=%.4f, AUROC=%.4f, AUPR=%.4f, F1=%.4f, trainTime=%.4f" % (e, args.epochs, loss, AUROC, AUPR, F1, epoch_time))
            ...
            cur_F1 = round(F1, 4)
            if cur_AUPR > round(best_aupr, 4) or cur_AUPR == round(best_aupr, 4) and cur_AUROC > round(best_auroc, 4):
                best_epoch, best_auroc, best_aupr, true, score, best_f1 = e, cur_AUROC, cur_AUPR, y_true, y_score, cur_F1
                # self.save_model()
            """
            self.save_history()
        ...
        log("best epoch %d, AUROC=%.4f, AUPR=%.4f, F1=%.4f" % (best_epoch, best_auroc, best_aupr, best_f1), color=Colors.MAGENTA)
        ...
        avg_train_time = np.mean(self.train_times)  # 计算所有轮次的平均训练时间
        print(f"Average training time per epoch: {avg_train_time:.4f} ms")
        return best_epoch, best_auroc, best_aupr, _result, best_f1, avg_train_time
...
if __name__ == '__main__':
    ...
    results_dict = {'ep': [], 'auroc': [], 'aupr': [], 'f1': [], 'time': []}  # 3*10*10
    for iter in range(1, 2):
        print(f"{Colors.RED}++++++++++++++++++++++ {str(iter)} iter START +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++{Colors.RESET}")
        time_results = {'ep': [], 'auroc': [], 'aupr': [], 'f1': [], 'time': []}
        for fold in range(1, 11):
            print(f"{Colors.YELLOW}======================== {str(fold)} fold start ============================================={Colors.RESET}")
            data = dataHandler.data[0][fold - 1]  # 第n折数据集，n=fold - 1
            # data = dataHandler.data[0][9]
            distanceMat = [dataHandler.data[1], dataHandler.data[2], data[-1]]  # rrmat，ddmat，拼接矩阵
            hope = Hope(args, data, distanceMat)  # 参数，第n折数据集，（rrmat，ddmat，拼接矩阵）
            ep, auroc, aupr, result, f1, trainTime = hope.run()
        ...
        mean_auroc_aupr_f1_time_4 = f"mean auc: {np.mean(time_results['auroc']):.4f}±{np.std(time_results['auroc']):.4f}, mean aupr: {np.mean(time_results['aupr']):.4f}±{np.std(time_results['aupr']):.4f}, mean f1: {np.mean(time_results['f1']):.4f}±{np.std(time_results['f1']):.4f}, mean time: {np.mean(time_results['time']):.4f}±{np.std(time_results['time']):.4f}"
        mean_auroc_aupr_f1_time_3 = f"mean auc: {np.mean(time_results['auroc']):.3f}±{np.std(time_results['auroc']):.3f}, mean aupr: {np.mean(time_results['aupr']):.3f}±{np.std(time_results['aupr']):.3f}, mean f1: {np.mean(time_results['f1']):.3f}±{np.std(time_results['f1']):.3f}, mean time: {np.mean(time_results['time']):.3f}±{np.std(time_results['time']):.3f}"
        with open(os.path.join('new_result', args.dataset + f'_{iter}iter_10folds.csv'), 'a') as f:
            f.write(f"{mean_auroc_aupr_f1_time_4}\n")
            f.write(f"{mean_auroc_aupr_f1_time_3}\n")
        ...
        print(f"{Colors.YELLOW}{iter} iter f1 list: {time_results['f1']}{Colors.RESET}")
        print(f"{Colors.YELLOW}{iter} iter time list: {time_results['time']}{Colors.RESET}")
        log(mean_auroc_aupr_f1_time_4, color=Colors.YELLOW)
        log(mean_auroc_aupr_f1_time_3, color=Colors.YELLOW)
        ...
        results_dict['f1'].extend(time_results['f1'])
        results_dict['time'].extend(time_results['time'])
        print(f"{Colors.RED}++++++++++++++++++++++ {str(iter)} iter END +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++{Colors.RESET}\n\n")
    ...
    log(f"Mean AUC: {np.mean(results_dict['auroc']):.4f}±{np.std(results_dict['auroc']):.4f}, Mean AUPR: {np.mean(results_dict['aupr']):.4f}±{np.std(results_dict['aupr']):.4f}, Mean F1: {np.mean(results_dict['f1']):.4f}±{np.std(results_dict['f1']):.4f}, Mean Time: {np.mean(results_dict['time']):.4f}±{np.std(results_dict['time']):.4f}",
        color=Colors.CYAN)
    log(f"Mean AUC: {np.mean(results_dict['auroc']):.3f}±{np.std(results_dict['auroc']):.3f}, Mean AUPR: {np.mean(results_dict['aupr']):.3f}±{np.std(results_dict['aupr']):.3f}, Mean F1: {np.mean(results_dict['f1']):.3f}±{np.std(results_dict['f1']):.3f}, Mean Time: {np.mean(results_dict['time']):.3f}±{np.std(results_dict['time']):.3f}",
        color=Colors.GREEN)
