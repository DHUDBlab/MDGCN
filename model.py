...
from utils import metaregular
from args import args


class MODEL(nn.Module):
    def __init__(self, drug_num, dis_num, rrMat, ddMat, rdMat, hide_dim, layer_num):
        ...
        rd_mat = self.rdMat[: self.drug_num, self.drug_num:]
    ...
    def metafortransform(self, auxi_embed_r, target_embed_r, auxi_embed_d, target_embed_d):
        ...
        tembed_rs = (t.sum(t.multiply(auxi_embed_r.unsqueeze(-1), low_weight_r1), dim=1))  # Equal to a two-layer linear network; Ciao and Yelp data sets are plus gelu activation function
    ...
    def forward(self, ifTraining, uid, iid, norm=1):
        ...
        metats_drug_embed, metats_dis_embed = self.metafortransform(drugEmbedding, rd_drugEmbedding, disEmbedding, rd_disEmbedding)
        ...
        return drugEmbedding, disEmbedding, (args.wr1 * rd_drugEmbedding + args.wr2 * drugEmbedding), (args.wd1 * rd_disEmbedding + args.wd2 * disEmbedding), rd_drugEmbedding, rd_disEmbedding, meta_reg_loss
